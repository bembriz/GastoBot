...
def check_prerequisites() -> list[dict]:
    ...
    if not creds_path:
        results.append(
            {
                "check": "GASTOSIA_GOOGLE_CREDENTIALS_PATH",
                "status": "failed",
        ...
        results.append(
            {"check": "credencial JSON", "status": "passed", "detail": f"encontrado: {creds_path}"}
        )

    if not sheet_id:
        results.append(
            {
                "check": "GASTOSIA_GOOGLE_SPREADSHEET_ID",
                "status": "failed",
                "detail": "variable no definida",
            }
        )
...
def validate_with_api(creds_path: str, sheet_id: str) -> list[dict]:
    ...
    try:
        ...
        results.append(
            {"check": "gspread instalado", "status": "failed", "detail": "pip install gspread"}
        )
        return results

    results.append({"check": "gspread instalado", "status": "passed", "detail": "disponible"})
    ...
    try:
        ...
        if has_control:
            ...
            results.append(
                {
                    "check": "_Control",
                    "status": "passed" if has_all else "partial",
        ...
        if len(row) == 16 and row[8] == "100.00":
            results.append(
                {"check": "escritura/lectura", "status": "passed", "detail": "datos integros"}
            )
        else:
            results.append(
                {
                    "check": "escritura/lectura",
                    "status": "failed",
        ...
        results.append(
            {"check": "limpieza", "status": "passed", "detail": f"pestaña '{test_name}' eliminada"}
        )
...
def main() -> int:
    ...
    if creds_path and sheet_id and Path(creds_path).exists():
        ...
        for r in api_results:
            status = (
                "OK" if r["status"] == "passed" else "WARN" if r["status"] == "partial" else "FAIL"
            )
            print(f"[{status}] {r['check']}: {r['detail']}")
    else:
